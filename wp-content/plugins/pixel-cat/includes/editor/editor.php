...
function fca_pc_settings_page() {
	...
	$html .= "<form novalidate style='display: none' action='' method='post' id='fca_pc_main_form' class='$form_class'>";
		...
		$html .= '<a class="fca-pc-top-icon" target="_blank" href="https://fatcatapps.com/convertcat/"><img height="120" width="120" src="' . FCA_PC_PLUGINS_URL . '/assets/pixelcat_icon_128_128_360.png' . '"></a>';

		$html .= '<h1>' .  __('ConvertCat', 'pixel-cat') . '</h1>';
	...
	<div id='fca-pc-event-modal' style='display: none;'>
		...
		<table class="fca_pc_modal_table">
			...
			<tr>
				...
				<td><span style='font-style: italic; position: relative; top: 7px;'><?php _e('Parameters, time delay, all events and dynamic event triggers<br>are available in', 'pixel-cat') ?> <a href='https://fatcatapps.com/convertcat' target='_blank'><?php _e('ConvertCat Pro', 'pixel-cat') ?></a>.</span></td>
			</tr>
		</table>
...
function fca_pc_add_premium_integrations( $options ) {
	...
	<table class='fca_pc_setting_table fca_pc_integrations_table'>
		...
		<tr>
			...
			<span class='fca_pc_hint'><?php _e("Send Lead event, plus custom events related to Quiz engagement to Facebook.", 'pixel-cat') ?> <a target='_blank' href='https://fatcatapps.com/convertcat/'><?php _e('Learn More...', 'pixel-cat') ?></a></span></td>
		</tr>
		<?php } ?>
	</table>
	<?php
...
function fca_pc_add_woo_integrations( $options ) {
			...
			<table class='fca_pc_integrations_table'>
				...
				<tr class='fca-pc-woo-feed-settings'>
					<th><?php _e('Feed URL', 'pixel-cat') ?></th>
						<td><input style='height: 35px;' type='text' onclick='this.select()' readonly value='<?php echo get_site_url() . '?feed=convertcat' ?>' >
	...
	} else if ( FCA_PC_PLUGIN_PACKAGE === 'Lite' ) {
		?>
		<div id='fca-pc-woo-table'>
			<h3><?php _e('WooCommerce Integration', 'pixel-cat') ?></h3>
			<h4><?php _e('Requires WooCommerce 3.0.0+ and ', 'pixel-cat') ?><a href='https://fatcatapps.com/convertcat/woocommerce' target='_blank'><?php _e( 'ConvertCat Pro', 'pixel-cat' ) ?></a>.</h4>
...
function fca_pc_marketing_metabox() {
	...
	<div id='fca-pc-marketing-metabox' style='display: none;'>
		<h3><?php _e( 'Get ConvertCat Pro', 'pixel-cat' ); ?></h3>
		...
		<div style='text-align: center'>
			<a href="https://fatcatapps.com/convertcat" target="_blank" class="button button-primary button-large"><?php _e('Upgrade & Boost My Conversions', 'pixel-cat'); ?></a>
		</div>
	</div>
	<?php
	return ob_get_clean();
